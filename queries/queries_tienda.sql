...
SELECT nombre, precio , ROUND(precio * 1.12, 2)
FROM producto;
...
/* 5. Llista el nom dels productes, el preu en euros i el preu en dòlars estatunidencs (USD).
 ...
 SELECT nombre 'nom_de_producto', precio 'euros', ROUND(precio * 1.12, 2) 'dolars'
 FROM producto;
...
SELECT nombre, ROUND(precio) 'Precio_redondeado'
FROM producto;

/* 10. Llista els noms i els preus de tots els productes de la taula producto, truncant el valor del preu per a mostrar-lo sense cap xifra decimal.*/
SELECT nombre, TRUNCATE(precio, 0) 'Precio_sin_decimales'
FROM producto;
